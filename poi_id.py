...
for numOfFeatures in range(1,18):
    ...
    dtGridSearch = GridSearchCV(dtclf, param_grid=dtParams, cv = cv, scoring='precision')
    ...
    cv = StratifiedShuffleSplit(labels,n_iter=10,random_state=42)
    svcGridSearch = GridSearchCV(svcclf, param_grid=svcParams, cv = cv, scoring='precision')
...
chosenCLF = SVC(gamma=0.0, tol=0.01, C=10.0, class_weight= 'auto', kernel='rbf', random_state=42)
